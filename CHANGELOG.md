...
- **An identity returning to a busy beam went permanently deaf: the hooks timed out on every prompt.** `check.sh` signature-verified every file newer than the cursor (openssl, ~75 ms each) *before* checking who it was for, and moved the cursor only after the whole scan. One identity with 855 unread files on the fleet beam took 1m44s per scan; Claude Code killed the 5 s prompt hook and the 10 s boot hook every time, zero progress was kept, and no message, title, watcher, or doorbell ever reached that session. Delivery is now bounded: a one-`grep` recipient pre-filter (only candidates are verified; own sends dropped), a per-run delivery cap (`BEAMS_DELIVERY_CAP`, default 20) and time budget (`BEAMS_SCAN_BUDGET_SECS`; 3 s on a prompt, 6 s at boot, 20 s interactive), and a *safe* partial cursor advance — the scan stops only where every unread candidate is strictly newer than the newest file reached, so nothing is lost or re-delivered. Capped runs say how much is still queued (`(+more queued)` in the banner); the next prompt continues. On the real 855-file backlog a prompt hook now finishes in ~3 s. New `tests/round-30.sh`.
