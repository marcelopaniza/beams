...
A message whose body happened to contain a bare `---` line failed its own signature check and vanished before reaching anyone; it now arrives intact on every delivery path. An identity coming back to a big backlog no longer forces one turn per five-second poll while it drains. Two smaller cleanups ride along: leaving a beam takes its `tie.*` cursor companions with it, and the watcher's notification caps the sender name like the native doorbell already did.

### Fixed

- **A message body containing a line that was exactly `---` — a markdown rule, a pasted message dump, a fake frontmatter block — failed signature verification and was silently dropped, on every delivery path, Claude and generic alike.** `beams::extract_body` (`lib/common.sh`) counted every `---`-only line as a frontmatter fence, even ones inside the body, so a body with such a line closed "frontmatter" a second time and the extracted body no longer matched the signature `send.sh` computed over the real one — the message failed `beams::msg_validate` and vanished before reaching any recipient. The body is now passed through verbatim, unconditionally, once the real front matter has closed (the first two fences). New `tests/round-35.sh` cases A–D (a `---` line mid-body, a body that is only `---`, a pasted fake-frontmatter block, and an ordinary message with no dashes at all, all delivered intact via `--human`, `--inject`, and `--notify`).

- **The native doorbell posted one summary per POLL, so an identity coming back to a big backlog forced one turn every 5-second poll until it drained** (855 messages at the 20-per-poll cap is 43 turns in a few minutes). `lib/watcher_daemon.sh`'s `post_batch_to_inbox` now holds back a drain — a batch that itself hits the 20-message cap — until `inbox_post_min_gap` (60s) has passed since the last such full post; a batch under the cap (an ordinary conversation) still posts the same poll it lands in, every time. wake.log, `--on-message`, and the desktop notification are unaffected — they still fire per message regardless of the native post's cooldown. `tests/round-35.sh` case E (a fresh identity facing a 50-message backlog under a 1s poll gets far fewer native posts than polls — at most 2, not one per poll — while a message sent right after the drain still posts within one poll).

- **`/beams:admin leave` left a beam's `tie.<cursor>` companion files behind after removing the cursors themselves.** `lib/leave.sh` now deletes both tie companions alongside the hook and notify cursors, so no local state about that beam's read history survives leaving it.

- **The watcher's `--notify` render had no length cap on the sender name.** `lib/check.sh` now caps it at 64 characters, after the control-character strip, matching the cap the native doorbell summary already applies to the same field.

- **`tests/round-33.sh` could fail its own cleanup when a watcher was still exiting.** The EXIT trap signalled the three watcher daemons and deleted the temp tree straight away, so a daemon still shutting down could re-create a file under `state/<sid>/` mid-delete and the resulting `rm: … Directory not empty` became the round's exit status — failing the round *after* every assertion had already passed. Cleanup now waits for the daemons to actually exit (SIGKILL after ~2 s), never lets the delete decide the status, and re-raises the status that triggered the trap so a genuine failure still fails the round. Assertions unchanged.

### Added

- **`tests/round-35.sh`** — proves the two fixes above together: a `---`-line-in-body message (mid-body, body-is-only-`---`, and a pasted fake-frontmatter block) and an ordinary message with no dashes all deliver intact via `--human`, `--inject`, and `--notify` (cases A–D); and a fresh identity facing a 50-message backlog under a 1s poll gets far fewer native doorbell posts than polls, while a message sent right after the drain still posts within one poll (case E).

## [0.12.0] — 2026-09-20

The real-time doorbell moves onto Claude Code's own session inbox socket — nothing left for the model to arm or re-arm. Delivery is now bounded so a big backlog can no longer time out the hooks, and the identity lease follows the Claude process instead of the session id.

### Fixed

- **The real-time doorbell was dead on current Claude Code (2.1.2xx).** The Monitor tool no longer has a `persistent` flag and rejects unknown arguments, every monitor is ended after at most 30 minutes, and the tool is loaded on demand (ToolSearch). The arm instruction asked for `persistent: true` and a 60-minute deadline and forbade a second call — so no session on a current harness could arm the bell at all, and a corrected call would have gone silent half an hour in. The instruction (`beams::doorbell_instruction`) now names only accepted arguments (`timeout_ms: 1800000`), tells the session to load Monitor via ToolSearch when it is not in its tool list, and to re-arm with the same call whenever the harness reports the doorbell expired or stopped. `tests/round-28.sh` asserts the new contract.

- **An identity returning to a busy beam went permanently deaf: the hooks timed out on every prompt.** `check.sh` signature-verified every file newer than the cursor (openssl, ~75 ms each) *before* checking who it was for, and moved the cursor only after the whole scan. One identity with 855 unread files on the fleet beam took 1m44s per scan; Claude Code killed the 5 s prompt hook and the 10 s boot hook every time, zero progress was kept, and no message, title, watcher, or doorbell ever reached that session. Delivery is now bounded: a one-`grep` recipient pre-filter (only candidates are verified; own sends dropped), a per-run delivery cap (`BEAMS_DELIVERY_CAP`, default 20) and time budget (`BEAMS_SCAN_BUDGET_SECS`; 3 s on a prompt, 6 s at boot or on the Stop hook, 20 s for the watcher and `/beams:read`), and a *safe* partial cursor advance — a run can stop anywhere and resumes exactly there next time, never losing or re-delivering a message. Capped runs say how much is still queued (`(+more queued)` in the banner); the next prompt continues. On the real 855-file backlog a prompt hook now finishes in ~3 s. New `tests/round-30.sh`.

- **That "safe" partial advance above could not actually stop inside a backlog where every file shared one mtime** (`rsync` without `-t`, `cp -r`, tar/zip extraction, a FAT/exFAT share) — the old rule only stopped where every unreached candidate was strictly newer than the newest one reached, which an equal-mtime group can never satisfy, so the budget and cap were silently ignored and the whole backlog ran in one hook call regardless. A cursor is now a PAIR: its mtime, plus a `tie.<cursor>` companion file (local state only, never the shared folder) naming the messages already consumed at exactly that mtime, so a run can stop mid-tie-group and resume exactly there, and a message that lands at the cursor's own mtime later is still delivered instead of silently skipped. The companion is trusted only while its own mtime still matches the cursor's. Two more edges closed in the same pass: the cursor advance is bounded by the scan's OWN directory listing, never a fresh one, so a message that lands between the scan and the cursor write is caught on the next run instead of swallowed; and a capped hook/human run no longer drags the watcher's notify cursor BACKWARDS, so it can't make the watcher — desktop ping and native doorbell alike — re-fire for mail the user already saw. New cases in `tests/round-30.sh` (60 messages sharing one mtime, bounded and lossless; a message landing mid-scan caught next run; a capped run never rewinds the notify cursor).

- **A message from someone else whose BODY quoted a `from: <our sid>` header line was silently dropped as one of our own sends, and the cursor advanced past it for good.** The self-send filter used to `grep` the WHOLE file for that line before the cursor moved; agents on this bridge paste message dumps at each other, so a quoted header in a body was easy to hit by accident. The check now reads only the header, up to the closing `---` fence, with the `read` builtin — cheaper than the grep it replaces, and a quoted header line inside a body can no longer masquerade as the real one. Covered by `tests/round-30.sh` case H.

- **A capped run that stopped before its first match went completely silent, and a hint could imply more precision than the scan actually had.** A budget or cap spent entirely on other recipients' mail, or inside the pre-filter, used to report nothing at all instead of the backlog it already knew was queued; and a run that skipped an entire beam wholesale (budget already gone before reaching it) could quote an exact-sounding remaining count that was really just a floor. `check.sh` now always surfaces the "more queued" hint when a run stops early — even having delivered zero — and drops to a vaguer "more" (no number) the moment any beam was skipped wholesale, so a number it does show is never an undercount. A capped run also leaves `state/partial-delivery` (local state only) until a later run finally drains everything, so `hooks/check-messages.sh`'s mtime fast path takes the slow path instead of reporting "nothing changed" while a backlog is still draining. Covered by `tests/round-30.sh` cases E, J, and K.

- **`--inject` and `--peek` had inherited that same 20-message/20-second default cap, and `--count` honoured an exported `BEAMS_DELIVERY_CAP` / `BEAMS_SCAN_BUDGET_SECS`.** Both surfaced while finishing the bounded-delivery work above, before either shipped: `--inject` is a generic (non-Claude) model's *only* delivery path — no hook, no `/beams:read` to catch the rest next turn — and `--peek` is a second identity's read-only preview of a beam (docs/CROSS-CLI.md), so silently capping either at 20 could lose messages a wrapped model would never see again; and an operator capping `--inject` for a wrapped model was able to skew the "exact" unread count `beams-react` and `/beams:status` trust. `lib/check.sh` now defaults `--inject`/`--peek` to unbounded (`0`/`0`, same as `--count`) while still honouring an explicit override, and forces `--count` back to exact regardless of either variable. New `tests/round-34.sh`.

- **A restart or `/clear` within the 15-minute lease window left the session "not initialised".** The lease proved a holder alive by scanning `/proc` for its session id — but only *child* processes carry `CLAUDE_CODE_SESSION_ID` (the `claude` process itself does not), so the long-lived watcher daemon, which inherited it, kept a dead session's name "busy" for as long as the daemon ran; and Claude Code mints a new session id on `/clear`, which then looked like a foreign live holder. The lease now records the holder's Claude **process** (`claude_pid` + start time, from `$CLAUDE_PID`): a new session id in the same process is recognised as the holder and rebinds silently (even when the project has several identities), a gone process frees the name at once, a recycled pid is detected by its start time, and a different live process still needs `--force`. The daemon is spawned without `CLAUDE_CODE_SESSION_ID` so it can never impersonate a session, and a new **SessionEnd** hook releases the lease on a real exit (kept on `clear`/`resume`). New `tests/round-31.sh`.

- **A lease with no `claude_pid` at all was read as proof the holder was gone.** `beams::_holder_gone` now answers `yes | no | unknown`, and only `yes` frees a name: with no `claude_pid` in the lease and no process carrying the holder's session id, the verdict is `unknown`, so the name stays busy for `BEAMS_INUSE_STALE_SECONDS` (default 900s) and needs `--force`. Reason: on a harness that exports no `$CLAUDE_PID`, the only processes that ever carry `CLAUDE_CODE_SESSION_ID` are the session's transient hooks, so "nothing carries it right now" is the normal state of a LIVE session, not evidence of death — the old reading handed a live session's name away without `--force`. A real exit needs no guess at all: the SessionEnd hook releases the lease. `tests/round-20.sh` case E and `tests/round-31.sh` case H assert the new contract.

- **The prompt hook's fast path never engaged.** `hooks/check-messages.sh` keyed its change-detection cache to the legacy single-config path (`~/.config/beams/config.json`), which a name-bound identity never has — so every prompt ran the full check, and a leftover legacy config would have driven the cache from the wrong beam list. The cache now follows the session's bound identity, yields to the slow path when the lease is older than 5 minutes (so the heartbeat still runs), and refuses a "hot" snapshot — one taken in the same second a beam directory was last written — because mtimes are compared at second granularity and a message landing later in that second would otherwise wait for the next change. Covered by `tests/round-30.sh` case C.

- **`/beams:start` told an initialised, name-bound terminal it was NEW.** Step 0 tested for a config file in the per-session dir, which a bound identity never has; it now asks `lib/status.sh`, which resolves the identity like every other command.

- **The `📬 beams` banner joined sender names with alternating `,` and ` `** (`paste -d ', '` cycles its delimiters): "game3,reporting felix,game4". Now "game3, reporting, felix, game4".

### Changed

- **Session start re-offers the doorbell whenever nothing is tailing the wake file** (`beams::doorbell_reader` — fuser, then lsof, then a `/proc/<pid>/fd` scan via a bash builtin `-ef` test when neither tool is on PATH — the same probe `/beams:status` and the mid-session arm use) instead of skipping `clear`/`compact` by source. With monitors capped at 30 minutes, a compaction almost always finds the bell gone — and it is exactly the event that drops the re-arm rule from the model's context. Where no probe method exists at all (no `fuser`, no `lsof`, no readable `/proc`) an armed monitor is invisible, so the hook falls back to the old source rule and stays silent on `clear`/`compact` instead of arming a duplicate doorbell.

### Added

- **The real-time doorbell now rides Claude Code's own session inbox — no Monitor, no 30-minute re-arm, survives `/clear`.** Claude Code ≥ 2.1.224 binds a Unix socket per Claude process (the session "inbox") and exports `CLAUDE_CODE_MESSAGING_SOCKET` + `CLAUDE_CODE_MESSAGING_TOKEN` to hooks and Bash commands; posting a line to it starts a new turn the instant an idle session receives it — a doorbell the harness rings itself, with nothing for the model to arm or re-arm. `hooks/check-on-start.sh` and `beams::doorbell_autostart` (`lib/common.sh`) now publish this session's socket, token, and `claude_pid_start` (the holder process's start time, as a lease claim records it) to a pointer file (`$BEAMS_CONFIG_DIR/inbox.json`, `0600`, next to `identity.key`) at every session start and on join/name; the watcher daemon reads that pointer at post time — never bakes the socket into its own long-lived environment — and after each poll that finds new mail, posts ONE batch summary straight into the socket (`beams::inbox_post`, `post_batch_to_inbox` in `lib/watcher_daemon.sh`): the reply clause, a line marking what `/beams:read` returns as other parties' data (not instructions), then one `- [beam] sender` line per message — identifiers only, no body preview — capped at 20, `+N more` beyond that. The socket write goes through `python3` with a `socat` fallback; `BEAMS_INBOX_POSTER=python3|socat` pins one client (unset/`auto` tries python3 then socat), and a pinned client never silently falls through to the other. `hooks/session-end.sh` drops the pointer (`beams::inbox_forget`) on a real exit and keeps it on `clear`/`resume`, where the same process and socket live on. The wake.log + Monitor path (v0.11.0) stays wired as the fallback — older harnesses have no socket, and a receiver configured `crossSessionInbound: refuse|hold` is now detected before publishing (`beams::inbox_allowed` reads only that one key — never a settings file's contents — from the user, project, and managed settings files, and any `refuse`/`hold` among them wins) and kept on the fallback too; a stale pointer is removed so neither the watcher nor `/beams:status` advertises a transport that will not deliver, and the project settings path is resolved through the plugin's own project-dir resolution (cwd fallback), since `CLAUDE_PROJECT_DIR` is never exported to a slash command's `!` block; only the harness's own remote kill-switch can't be detected, and it behaves like a dead socket. `/beams:status` now reports `doorbell: native (session inbox socket <path>; watcher pid <N>)` when the pointer's socket is live and the watcher is actually running. New `tests/round-32.sh`, now cases A–N (pointer publish incl. a planted-symlink guard, an end-to-end post, batching five messages into one frame, a dead-socket fallback, native-vs-Monitor mode selection, SessionEnd cleanup, the `crossSessionInbound` refuse/hold/accept matrix — including a project-level refuse and one with `CLAUDE_PROJECT_DIR` unset — a no-body-text/forged-summary-line case, `/beams:watch start` publishing, both `/beams:status` native texts, the socat poster, the pinned-identity ownership case, and the SessionEnd ownership + orphan cases).

- **`lib/watch.sh` now publishes the inbox pointer on `start`/`restart` too** (`beams::inbox_publish`), not just at session start or on join/name — so a session that opted out of `watch_on_boot`, or that created its identity mid-session, still gets native delivery from a manual `/beams:watch start`. Never changes that command's output or exit status.

- **`beams::inbox_publish` now refuses unless the caller is a real Claude session AND no explicit `BEAMS_CONFIG_DIR` override is in the environment.** Both messaging variables (`CLAUDE_CODE_MESSAGING_SOCKET`/`_TOKEN`) are inherited by every child of a session, so a generic cross-CLI rider driven from inside a Claude Bash call (`BEAMS_CONFIG_DIR=<other-identity> beams name x`) previously published the HUMAN session's socket under its own identity — its watcher then rang the human's session forever with mail `/beams:read` could not find there. Consequence: a Claude session whose own environment exports `BEAMS_CONFIG_DIR` stays on the Monitor fallback by design.

- **`beams::inbox_forget` (the SessionEnd drop) now removes only a pointer that is OURS or ORPHANED.** OURS = it names this session id, or this very Claude process after a `/clear` rotated the id. ORPHANED = its pid is gone, or its recorded start time no longer matches that pid. Before, an ousted session's exit unlinked the pointer a `/beams:name <x> --force` taker had just published, silencing a live session that had no way to republish before its next SessionStart.

- **`tests/round-33.sh`** (the doorbell across three named sessions bound in one project folder: each gets its own process lease, published inbox pointer, fake socket, and watcher daemon; messages addressed to one, to `all`, or by `@`-mention land with the exact per-identity total and never cross into another identity's socket or wake.log; a `/clear`-style rotation keeps the same socket; `SessionEnd` for one identity releases only that identity's pointer and lease) and **`tests/round-34.sh`** (generic-model parity with no Claude environment at all: a 30-message backlog drains in one `beams read --inject` call, a `BEAMS_DELIVERY_CAP`-capped drain loses nothing across repeated calls, `--count` stays exact through a partial drain and against an exported cap, `--peek` defaults unbounded, `beams-react` drains a 30-message backlog in one fire, every documented addressing form delivers correctly, a generic identity rebinds its name without `--force` across a fresh subshell, and its watcher writes wake.log but never attempts an inbox post).

- **`SessionEnd` hook (`hooks/session-end.sh`)** — releases this terminal's identity lease when the session really ends (`logout`, `prompt_input_exit`, `other`), so the next session in the project can take the name at once; kept on `clear` and `resume`, where the same process lives on and rebinds by pid.

- **Delivery knobs:** `BEAMS_DELIVERY_CAP` (messages per run) and `BEAMS_SCAN_BUDGET_SECS` (seconds per run); `0` = unlimited. Defaults vary by mode: 3 s on a prompt / 6 s at session start / 6 s on the Stop hook / 20 s for the watcher and `/beams:read`, each capped at 20 messages; `--inject` and `--peek` default unbounded (`0`/`0`) but still honour an explicit override. `--count` is always exact and immune to both, however they're set.

- **`tests/round-30.sh`** (bounded delivery: exact recipient pre-filter, cap with a safe partial cursor — no loss, no duplicate — identity-keyed hook fast path, sender-list join) and **`tests/round-31.sh`** (process-based lease: `/clear` rebind, restart reclaim, live-process protection, pid-reuse guard, `SessionEnd` release, daemon environment).

- **Joining arms the doorbell in the same session — no restart needed.** `/beams:join`, `/beams:name` (a mid-session bind like "join beams as `<name>`"), and a profile `init` now call `beams::doorbell_autostart`: the watcher daemon is started armed with the wake-file hook (idempotent `start` — a healthy daemon, or one carrying a custom `--on-message`, is left alone), and the Monitor-arm instruction is printed into the command output for the model to follow. Previously all of this only happened at the next SessionStart, so the very session that ran `/beams:start` stayed pull-only until a restart. Duplicate-doorbell guard: the instruction is suppressed whenever a live process already tails `wake.log` (the same open-reader probe — `fuser`, `lsof`, or `/proc`) — which is exactly the state after a `/clear`, where the process keeps its monitors but loses the context — and outside Claude Code (no Monitor tool there; the watcher still starts for cross-CLI setups). Same opt-outs as boot (`react.watch_on_boot: false` / `BEAMS_DISABLE_WATCH_ON_BOOT=1`). The instruction text now lives in one place (`beams::doorbell_instruction` in `lib/common.sh`), shared by the SessionStart hook and the mid-session arm so the two can't drift. The wizard's Step 4 notification question is gone — nothing left to ask. New `tests/round-29.sh`.

- **`/beams:status` now shows doorbell ground truth.** A new `doorbell:` line reports `armed (wake.log reader pid N)` or `NOT armed — …re-offer…` using the same open-reader probe (`beams::doorbell_reader` — `fuser`, `lsof`, or a `/proc/<pid>/fd` scan, whichever the host has) the mid-session arm uses — so "is the bell actually live?" is one command instead of guesswork.
...
- **The real-time doorbell now wakes idle sessions with a bone-stock Claude Code launch — no flags, on by default.** New transport: the watcher's `--on-message` hook (`lib/on-message.sh`) appends one sanitized line per new message to the identity's wake file (`$BEAMS_CONFIG_DIR/wake.log`), and the SessionStart hook asks the session to arm a persistent **Monitor** task (`tail -n 0 -F` on that file) — the harness turns each appended line into an event that re-invokes the session even when it is fully idle; the woken session runs `/beams:read`. Idle cost stays **zero tokens** (the waiting is a detached `tail`, not a model loop and not a Stop-hook sustain); a delivered message costs the one turn that reads it. Same opt-out as the watcher (`react.watch_on_boot: false` / `BEAMS_DISABLE_WATCH_ON_BOOT=1`). New `tests/round-28.sh` (append/sanitize/cap unit cases; SessionStart truncate + re-arm + instruction; compact suppression; opt-out; end-to-end send→wake line).

- **The responder profile now means autonomous replies out of the box.** A session initialized with `--profile responder` carries `role: responder` in its config, and the doorbell-arm instruction tells it at every boot to handle incoming beams and reply on the beam itself — no human in the middle, no hand-written instructions. Destructive, deploy, or production-touching requests arriving by beam are still surfaced to the user instead of obeyed. New **docs/AUTONOMY.md** carries the recipes (new session: one command; existing session: a paste-in block).
