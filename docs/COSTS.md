...
- **Wake-file doorbell (default-on, v0.11 — replaces the channel server).** The watcher appends one line per new message to a local wake file (`$BEAMS_CONFIG_DIR/wake.log`); the session's persistent `Monitor` task (armed once, at the session's first prompt) turns each line into a wake of the *idle* session. **0 model tokens while idle** — the waiting is a detached `tail`, not a model loop. A delivered message costs **one turn**: the same turn that reads and surfaces it (and replies, when the session's role calls for that). Off whenever the watcher is off.
