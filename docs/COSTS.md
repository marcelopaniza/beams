...
- **Native doorbell — session inbox socket (default-on, current Claude Code).** On Claude Code ≥ 2.1.224 the watcher posts straight into the session's own inbox socket instead (see [INTERNALS.md § Real-time doorbell](INTERNALS.md#real-time-doorbell-native-transport)): after each poll that finds new mail, ONE batch summary — capped at 20 message lines — goes to the socket, and the harness starts a turn on its own when the session is idle. **0 model tokens while idle**, and a whole batch still costs **one turn**, whatever its size — nothing to arm, nothing to re-arm. A drain — many full (20-message) batches back to back, e.g. an identity returning to a big backlog — is throttled to at most one turn per minute; a non-full batch (an ordinary conversation, or the tail end of a drain once it drops below the cap) is never throttled and always posts the same poll it lands in. So draining N messages costs roughly one turn per minute until the backlog clears, plus one more for whatever's left in that final, non-full batch.
- **Wake-file doorbell (fallback — older Claude Code, or a receiver set to `crossSessionInbound: refuse|hold`).** The watcher appends one line per new message to a local wake file (`$BEAMS_CONFIG_DIR/wake.log`); the session's `Monitor` task — armed at the session's first prompt, or on the spot when it joins/binds mid-session, and re-armed by the session at each 30-minute deadline — turns each line into a wake of the *idle* session. **0 model tokens while idle** — the waiting is a detached `tail`, not a model loop. A delivered message costs **one turn**: the same turn that reads and surfaces it (and replies, when the session's role calls for that). Off whenever the watcher is off.
