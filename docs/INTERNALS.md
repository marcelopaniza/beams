...
- **Cursors live per-session** in `$BEAMS_CONFIG_DIR/state/<sid>/` — never on the share. Two terminals can have completely different read positions without colliding. Each cursor is really a PAIR: the file's own mtime, plus a `tie.<cursor>` companion (same directory) naming the messages already consumed at exactly that mtime — so a backlog where many files share one mtime (`rsync` without `-t`, `cp -r`, tar/zip extraction, a FAT/exFAT share) can still be split safely across runs instead of landing as one indivisible blob. The companion is trusted only while its own mtime still matches the cursor's, so a clamp or a peer touch that re-stamps the cursor can't hide messages behind a stale name list. `/beams:admin leave` (`lib/leave.sh`) deletes a beam's tie companions alongside its two cursors, so no local state about that beam's read history survives leaving it.
...
- **Delivery is bounded per run, by mode, and safe to stop anywhere.** `check.sh` pre-filters new files for this recipient with one `grep` (only candidates pay for signature verification; a candidate's HEADER is read first, up to its closing `---` fence, to drop the session's own sends before any signature check or full read — so a message whose BODY quotes a raw `from:` line is never mistaken for one), then delivers within a per-mode time budget and message cap — both overridable via `BEAMS_SCAN_BUDGET_SECS` / `BEAMS_DELIVERY_CAP` (`0` = unlimited). Defaults: 3 s on a prompt, 6 s at session start or on the Stop hook, 20 s for the watcher and `/beams:read` — each capped at 20 messages; `--inject` and `--peek` default unbounded (a generic model's only delivery path, and a read-only preview, should never silently lose messages) but still honour an explicit override; `--count` is always exact and immune to both. Thanks to the cursor pair above, a capped run can stop and resume anywhere in a beam's (mtime, name) order — even mid-way through a same-mtime backlog — never past what its own directory listing actually saw (a message landing mid-scan is caught next run, not swallowed) and never backwards (so a capped hook/human run can't drag the watcher's own notify cursor, normally further ahead, back into re-firing for mail it already showed). A capped run leaves `state/partial-delivery` (local state only) so the prompt hook's fast path takes the slow path until a later run finally drains everything; the "more queued" hint always fires when a run stops early — even having delivered nothing — and only states an exact number when the scan is sure of it, falling back to a vaguer "more" rather than risk an undercount once any beam was skipped wholesale. A month-long backlog really does drain across a few prompts now, instead of timing out the hook forever.
