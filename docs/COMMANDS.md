...
`start` and `restart` also (re-)publish this session's native-doorbell pointer (see [INTERNALS.md § Real-time doorbell](INTERNALS.md#real-time-doorbell-native-transport)) — the way to get native delivery for a session that opted out of `watch_on_boot`, or that created its identity after boot.
...
> ⚠️ **Wrong tool for AI responder agents.** `--on-message` only exposes a 120-character preview via env vars. If you want an AI to *reason* about full message bodies and reply on the beam, you want **[`bin/beams-react`](CROSS-CLI.md#building-a-responder-agent)** instead — it polls, gates on `unread > 0`, pipes the full inbox into the wrapped AI via `bin/beams read --inject` (unbounded by default, so a big backlog drains in one shot), holds a single-instance lock, rate-limits, and ships with a refuse-destructive-ops directive. `--on-message` is for **notifications, webhooks, bells, log scribbling** — anything that doesn't need the message body, just the fact that a message arrived.
...
**What `--on-message` does.** It runs a shell command once per new message — nothing more. As of 0.11 the SessionStart hook — and, mid-session, a `/beams:join` or `/beams:name` — arms it automatically with `lib/on-message.sh`, the wake-file doorbell: each message appends one line to `$BEAMS_CONFIG_DIR/wake.log`, and the session's Monitor task (re-armed by the session at each 30-minute deadline) turns that line into a real-time wake of the idle session. On current Claude Code (≥ 2.1.224) the daemon also posts one batch summary per poll straight into the session's own inbox socket — the harness starts the turn itself, nothing to arm — and wake.log plus the Monitor task become the fallback, used only on older harnesses or when the receiver is set to `crossSessionInbound: refuse|hold` (see [INTERNALS.md § Real-time doorbell](INTERNALS.md#real-time-doorbell-native-transport)). `/beams:status` reports which one is live: `doorbell: native (session inbox socket <path>; watcher pid <N>)` when a watcher is actually posting into it (a live pointer with no running watcher gets a `run /beams:watch start` nudge instead), `armed (wake.log reader pid N)` for the fallback, or `NOT armed — …`. Passing your own `--on-message` replaces the stock hook for the current daemon run only (the next session start re-arms the stock one — set `react.watch_on_boot: false` for full manual control). For an interactive Claude session, the `UserPromptSubmit` hook still delivers messages on the next user prompt (zero tokens). For an autonomous responder agent that needs full message context, see the prominent callout at the top of this section: use [`bin/beams-react`](CROSS-CLI.md#building-a-responder-agent).
